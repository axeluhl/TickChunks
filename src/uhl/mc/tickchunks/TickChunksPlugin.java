...
public class TickChunksPlugin extends JavaPlugin {
	        	...
	        	if (world instanceof WorldServer) { // what else...?
					...
					if (args[0].toLowerCase().startsWith("k")) {
						...
						if (playerChunk.isKeptTickingEvenIfEmpty()) {
							...
							sender.sendMessage("Chunk ("+x+","+z+") is now being kept in PlayerChunkMap even when no player has it in sight.");
							final Plugin keepChunksPlugin = Bukkit.getServer().getPluginManager().getPlugin("KeepChunks");
							if (keepChunksPlugin != null) {
								if (debug) {
									getLogger().info("Found KeepChunks plugin. Requesting chunk ["+playerChunk.chunk.locX+
											","+playerChunk.chunk.locZ+"] to keep loaded");
								}
								String commandLine = "kc kc "+x+" "+z+" "+((WorldServer) world).getWorldData().getName();
								final boolean keepChunksResult = Bukkit.getServer().dispatchCommand(sender,
										commandLine);
								if (debug) {
									getLogger().info("KeepChunks command \""+commandLine+"\" returned "+keepChunksResult);
								}
							}
						}
						...
						if (!playerChunk.isKeptTickingEvenIfEmpty()) {
							...
							sender.sendMessage("Chunk ("+x+","+z+") is now being released from PlayerChunkMap when no player has it in sight.");
							if (Bukkit.getServer().getPluginManager().getPlugin("KeepChunks") != null) {
								sender.sendMessage("Consider releasing the chunk also from the KeepChunks plugin using command /kc rc "+x+" "+z+
	...
	public void onEnable() {
		...
		for (final Entry<Integer, Set<ChunkCoordIntPair>> chunkCoordinatesForWorldDimension : chunksToKeepTicking.getTickingChunksPerWorldDimension()) {
			@SuppressWarnings("deprecation") // how else can we obtain the server without using a deprecated method?
			final WorldServer worldServer = MinecraftServer.getServer().getWorldServer(chunkCoordinatesForWorldDimension.getKey());
	...
	public void onLoad() {
		...
		try {
			...
			getLogger().warning("The data file "+dataFile+" just existed but still cannot be found. Not loading any data.");
		}
	}
