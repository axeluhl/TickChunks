...
/**
 * Manages the collection of chunk coordinates that are to be kept ticking.
 * Such an object can be constructed from a {@link YamlConfiguration} object
 * if such object was produced by an instance of this class.
 */
public class ChunksToKeepTicking {
	...
	void load(YamlConfiguration data) {
		...
		for (final String dimensionNumberAsString : coordinates.getKeys(/* deep */ false)) {
			...
			for (final Map<?, ?> coordinate : coordinates.getMapList(dimensionNumberAsString)) {
				final ChunkCoordIntPair c = new ChunkCoordIntPair(((Number) coordinate.get(X_KEY)).intValue(), ((Number) coordinate.get(Z_KEY)).intValue());
				chunkCoordinatePairs.add(c);
			}
			coordinatesOfChunksToKeepTickingPerWorldDimension.put(Integer.valueOf(dimensionNumberAsString), chunkCoordinatePairs);
		}
	}
	...
	YamlConfiguration getData() {
		...
		synchronized (coordinatesOfChunksToKeepTickingPerWorldDimension) {
			for (final Entry<Integer, Set<ChunkCoordIntPair>> c : coordinatesOfChunksToKeepTickingPerWorldDimension.entrySet()) {
